...
<div class="card">
	<div class="card-body">
		<h6 class="card-title text-info">Edit Promotion Details</h6>
			<form id="myValidForm" action="{{route('promotion.details.update',$promotion_details->id)}}" method="post">
				@csrf
				<div class="row">
					<!-- Col -->
                    <div class="col-sm-6">
                        <div class="mb-3 form-valid-groups">
                            ...
                            <select class="form-select" name="promotion_id" aria-invalid="false">
                                ...
                                <option value="{{$promotion->id}}" {{$promotion->id == $promotion_details->Promotion_id  ? 'selected' : ''}}>{{$promotion->promotion_name}}</option>
                                @endforeach
                            </select>
                        </div>
					</div>
                    <div class="col-sm-6">
                        <div class="mb-3 form-valid-groups">
                            ...
                            <select class="form-select" name="Product_id" aria-invalid="false">
                                ...
                                <option value="{{$products->id}}" {{$products->id ==$promotion_details->Product_id ? 'selected' : ''}}>{{$products->name}}</option>
                                @endforeach
                    ...
                    <div class="col-sm-6">
						<div class="mb-3 form-valid-groups">
							...
							<input type="number" name="additional_conditions" class="form-control field_required" value="{{$promotion_details->additional_conditions}}" placeholder="Enter Addional Condition">
						</div>
					</div>
				</div><!-- Row -->
				<div>
